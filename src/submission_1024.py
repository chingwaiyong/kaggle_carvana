...
df_test = pd.read_csv('../input/carvana-image-masking-challenge/sample_submission.csv')
...
model = load_model(
    filepath='../input/model_weights.hdf5',
...
def data_loader(q, ):
    for start in range(0, len(ids_test), BATCH_SIZE):
        ...
        for id in ids_test_batch.values:
            img = cv2.imread('../input/carvana-image-masking-challenge/test/{}.jpg'.format(id))
...
df.to_csv('full1024_submission.csv.gz', index=False, compression='gzip')